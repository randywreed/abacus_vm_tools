...
def normalize_usage(usage: dict) -> dict:
    prompt = reported_number(usage, ("prompt_tokens", "input_tokens", "promptTokens", "inputTokens", "input", "prompt"), True)
    completion = reported_number(usage, ("completion_tokens", "output_tokens", "completionTokens", "outputTokens", "output", "completion"), True)
    total = reported_number(usage, ("total_tokens", "totalTokens", "total"), True)
    if total is None and prompt is not None and completion is not None:
        total = prompt + completion
    model_calls = reported_number(usage, ("model_calls", "modelCalls", "calls"), True)
